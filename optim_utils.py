...
def get_dataset(args):
    if 'laion' in args.dataset:
        ...
        with open('/scratch/psm12/water/dataset/mscoco_500_prompts.json') as f:
            raw_dataset = json.load(f)
        # Use only the first caption as the prompt for each sample
        dataset = []
        for entry in raw_dataset:
            first_prompt = entry['captions'][0] if entry['captions'] and entry['captions'][0].strip() else ''
            dataset.append({'image_id': entry['image_id'], 'prompt': first_prompt})
        prompt_key = 'prompt'  # Now each entry has 'prompt' as a single string
...
def inject_watermark(init_latents_w, watermarking_mask, gt_patch, args):
    ...
    if args.w_injection == 'complex':
        # Ensure gt_patch has the same dtype as init_latents_w_fft
        gt_patch_compatible = gt_patch.to(init_latents_w_fft.dtype)
        init_latents_w_fft[watermarking_mask] = gt_patch_compatible[watermarking_mask].clone()
    elif args.w_injection == 'seed':
        # Ensure gt_patch has the same dtype as init_latents_w
        gt_patch_compatible = gt_patch.to(init_latents_w.dtype)
        init_latents_w[watermarking_mask] = gt_patch_compatible[watermarking_mask].clone()
...
def calculate_detection_score(reversed_latents, watermarking_mask):
    """Calculate Tree-Ring detection score (sum of magnitudes in ring region)"""
    fft = torch.fft.fftshift(torch.fft.fft2(reversed_latents), dim=(-1, -2))
    masked_fft = fft[watermarking_mask]
    return torch.abs(masked_fft).sum().item()

def calculate_threshold(null_scores):
    """Compute threshold as mean + 3σ of null scores"""
    mu = np.mean(null_scores)
    sigma = np.std(null_scores)
    return mu + 3 * sigma
